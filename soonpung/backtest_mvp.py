...
def run() -> dict:
    ...
    for state in ("bull", "bear", "sideways"):
        ...
        cum_s = float((1 + sr).prod() - 1)
        cum_b = float((1 + br).prod() - 1)
        decomp[state] = {
            "days": int(mask.sum()),
            "순풍_total_ret": round(cum_s, 4),
            "SOXL_total_ret": round(cum_b, 4),
            "순풍_mdd": round(mdd((1 + sr).cumprod()), 4),
            "SOXL_mdd": round(mdd((1 + br).cumprod()), 4),
            "capture": round(cum_s / cum_b, 3) if abs(cum_b) > 1e-9 else None,
        }
    ...
    summary = {
        ...
        "note": "합성 SOXL 역연장 포함 — 역사 구간은 참고용(과신 금지).",
    }
